...
def _sinusoidal_positions(length, hidden_dim):
    ...
    angles = positions / torch.pow(
        10_000.0, 2 * torch.div(dimensions, 2, rounding_mode="floor") / hidden_dim
    )
...
class ACTPolicy(nn.Module):
    ...
    def __init__(self, config, *, load_backbone_weights=None):
        ...
        self.image_backbone = ResNet18Backbone(pretrained=bool(load_backbone_weights))
        ...
        encoder_layer = PositionalEncoderLayer(
            hidden, config.attention_heads, config.feedforward_dim, config.dropout
        )
        ...
        self.register_buffer(
            "posterior_position", _sinusoidal_positions(config.chunk_size + 2, hidden)
        )
    ...
    def _encode_posterior(self, qpos, actions, is_pad):
        ...
        encoded = self.posterior_encoder(source, position, padding_mask=padding_mask)
    ...
    def _encode_observation(self, qpos, images, latent):
        ...
        features = self.image_backbone(
            normalized.reshape(batch_size * camera_count, *images.shape[2:])
        )
        features = self.image_projection(features)
        positions = self.image_position(features)
        _, _, height, width = features.shape
        features = features.reshape(
            batch_size, camera_count, self.config.hidden_dim, height, width
        ).permute(0, 1, 3, 4, 2)
        positions = positions.reshape(
            batch_size, camera_count, height, width, self.config.hidden_dim
        )
    ...
    def forward(self, qpos, images, actions=None, is_pad=None):
        ...
        if actions is None:
            ...
            latent, mean, log_variance = self._encode_posterior(qpos, actions, is_pad)

        memory, memory_position = self._encode_observation(qpos, images, latent)
        query_position = self.action_queries.weight[None].expand(batch_size, -1, -1)
        target = torch.zeros_like(query_position)
        decoded = self.action_decoder(target, memory, query_position, memory_position)
        return {
    ...
    def loss(self, batch, *, kl_weight=10.0):
        ...
        output = self(batch["qpos"], batch["images"], batch["actions"], batch["is_pad"])
        valid = (~batch["is_pad"]).unsqueeze(-1)
        l1 = (torch.abs(output["actions"] - batch["actions"]) * valid).mean()
        mean = output["mean"]
        log_variance = output["log_variance"]
        kl = (
            -0.5 * (1.0 + log_variance - mean.square() - log_variance.exp()).sum(dim=-1)
        ).mean()
