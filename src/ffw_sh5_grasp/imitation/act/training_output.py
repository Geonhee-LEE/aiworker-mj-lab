...
def write_metrics(history, metrics_dir):
    ...
    with (metrics_dir / "metrics.jsonl").open(
...
def plot_metric(history, name, path):
    ...
    _mark_best_validation(draw, series, name, margin, height, xscale, yscale,
                          low)
...
def _mark_best_validation(
        draw, series, name, margin, height, xscale, yscale, low):
    if name != "loss":
        return
    validation = next((
