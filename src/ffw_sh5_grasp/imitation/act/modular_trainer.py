"""Isolated ACT trainer for controlled joint/task representation studies."""
...
from .dataset_loader import episode_paths, save_stats, split_episodes
from .modular_dataset_loader import (
    ModularACTEpisodeDataset,
    compute_modular_stats,
)
from .modular_representations import create_representation
from .modular_training_config import ModularACTTrainingConfig
...
def _base_checkpoint(config, policy_config, representation, *, epoch,
                     global_step, validation_loss, model_state):
    result = {
        "model": model_state,
        "epoch": int(epoch),
    ...
    if representation.name == "joint":
        result["policy_indices"] = tuple(RIGHT_POLICY_INDICES)
    return result
...
def train_modular(config_path):
    """Train one isolated representation run and return its output directory."""
    config = ModularACTTrainingConfig.load(config_path)
    _seed_everything(config.training_seed)
    representation = create_representation(config.representation)
    ...
    stats = compute_modular_stats(train_paths, representation)
    ...
    train_dataset = ModularACTEpisodeDataset(
        train_paths, **dataset_options)
    validation_dataset = ModularACTEpisodeDataset(
    ...
    device_name = (str(device) if device.type != "cuda" else
                   f"{device} ({torch.cuda.get_device_name(device)})")
    print(
        f"modular training representation={representation.name} "
        ...
        for epoch in range(config.epochs):
            ...
            if save_checkpoint:
                last = _base_checkpoint(
                    config, policy_config, representation,
                ...
                best = _base_checkpoint(
                    config, policy_config, representation,
                    epoch=best_epoch, global_step=best_global_step,
            ...
            if (completed % config.plot_interval == 0
                    or completed == config.epochs):
                for name in ("loss", "l1", "kl", "learning_rate"):
                    plot_metric(
                        history, name,
                        config.run_dir / f"plots/{name}.png")
