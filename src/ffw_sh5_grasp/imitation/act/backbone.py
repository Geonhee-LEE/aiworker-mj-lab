    ...
    def __init__(self, *, pretrained=True):
        ...
        try:
            ...
            raise ImportError(
                ...
                "install requirements-imitation.txt"
            ) from error
...
class PositionEmbeddingSine2D(nn.Module):
    ...
    def forward(self, feature_map):
        ...
        ones = torch.ones(
            (batch, height, width), dtype=torch.bool, device=feature_map.device
        )
        ...
        frequencies = torch.arange(
            self.feature_dim, dtype=torch.float32, device=feature_map.device
        )
        frequencies = self.temperature ** (
            2 * torch.div(frequencies, 2, rounding_mode="floor") / self.feature_dim
        )

        x_angles = x_position[..., None] / frequencies
        y_angles = y_position[..., None] / frequencies
        x_embedding = torch.stack(
            (x_angles[..., 0::2].sin(), x_angles[..., 1::2].cos()), dim=-1
        ).flatten(3)
        y_embedding = torch.stack(
            (y_angles[..., 0::2].sin(), y_angles[..., 1::2].cos()), dim=-1
        ).flatten(3)
        return torch.cat((y_embedding, x_embedding), dim=-1).to(dtype=feature_map.dtype)
