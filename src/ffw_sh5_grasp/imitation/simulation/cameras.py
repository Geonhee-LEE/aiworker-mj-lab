...
class MujocoCameraManager:
    ...
    def __init__(self, model, data, *, width=320, height=240,
                 camera_names=DEFAULT_CAMERA_NAMES, render_context=None,
    ...
    def _render_shared_context(self):
        ...
        try:
            mujoco.mjr_setBuffer(
                mujoco.mjtFramebuffer.mjFB_OFFSCREEN, context)
            for name in self.camera_names:
                ...
                mujoco.mjv_updateScene(
                    self.model, self.data, self._scene_option, None, camera,
                    mujoco.mjtCatBit.mjCAT_ALL, self._shared_scene)
                image = np.empty(
            ...
            mujoco.mjr_setBuffer(
                mujoco.mjtFramebuffer.mjFB_WINDOW, context)
        return images
