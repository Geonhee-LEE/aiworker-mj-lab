...
class MujocoRobotRerunLogger:
    ...
    def _color(self, geom_id):
        ...
        rgba = (
            self.model.mat_rgba[material_id]
            if material_id >= 0
            else self.model.geom_rgba[geom_id]
        )
        display_rgba = np.array(rgba, copy=True)
        display_rgba[:3] = np.minimum(display_rgba[:3], MAX_RERUN_COLOR_INTENSITY)
    ...
    def log_geometry(self):
        ...
        for geom_id in self.geom_ids:
            ...
            if geom_type == mujoco.mjtGeom.mjGEOM_MESH:
                ...
                self.recording.log(
                    path, rr.Ellipsoids3D(radii=float(size[0]), colors=color)
                )
            elif geom_type in (
                ...
                self.recording.log(
                    path,
                    rr.Cylinders3D(
                        lengths=2.0 * float(size[1]), radii=float(size[0]), colors=color
                    ),
                )
