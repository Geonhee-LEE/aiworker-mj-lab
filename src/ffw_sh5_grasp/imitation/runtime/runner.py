...
class TaskSpaceTemporalAggregator(TemporalAggregator):
    ...
    def action(self, timestep):
        ...
        quaternions = np.stack([normalize_quaternion(value[3:7]) for value in values])
...
class ACTPolicyRunner:
    def __init__(
        ...
        *,
        ...
        checkpoint_representation = (
            str(checkpoint.get("representation", "joint")).strip().lower()
        )
        if checkpoint_representation not in ("joint", "task"):
            raise ValueError(
                f"unsupported checkpoint representation: {checkpoint_representation!r}"
            )
        if (
        ...
        try:
            ...
            raise ValueError(
                ...
                "retrain it with the current config"
            ) from error
        self.policy = ACTPolicy(self.config, load_backbone_weights=False).to(
            self.device
        )
        ...
        if self.representation == "joint":
            ...
            raise ValueError("task policy must use 8D EE pose plus grasp state/action")
        else:
        ...
        for name in ("qpos_mean", "qpos_std", "action_mean", "action_std"):
            ...
            if np.asarray(getattr(self.stats, name)).shape != (expected,):
                raise ValueError(f"dataset statistic {name} does not match checkpoint")
    ...
    def _inputs(self, observation):
        ...
        if full_qpos.shape != (ACTION_DIM,):
            raise ValueError(
                f"policy qpos must have shape ({ACTION_DIM},), got {full_qpos.shape}"
            )
        if self.representation == "joint":
            ...
            try:
                ...
                raise ValueError(
                    "task policy observation requires ee_pose/right"
                ) from error
            if right_pose.shape != (7,) or not np.all(np.isfinite(right_pose)):
                raise ValueError("task policy right EE pose must be a finite 7D vector")
            right_pose = right_pose.copy()
            right_pose[3:7] = normalize_quaternion(right_pose[3:7])
            state = np.concatenate(
                (
                    right_pose,
                    full_qpos[[RIGHT_GRASP_INDEX]],
                )
            )
        ...
        if missing:
            raise ValueError(
                f"policy observation is missing cameras: {sorted(missing)}"
            )
        camera_images = [
        ...
        invalid = [
            name
            for name, image in zip(self.camera_names, camera_images)
        ...
        images = (
            np.stack([image.transpose(2, 0, 1) for image in camera_images]).astype(
                np.float32
            )
            / 255.0
        )
    ...
    def predict_chunk(self, observation):
        ...
        if self.representation == "task":
            ...
            predicted[:, 3:7] = np.stack(
                [normalize_quaternion(quaternion) for quaternion in predicted[:, 3:7]]
            )
            return predicted, output["is_pad"][0].sigmoid().cpu().numpy()
        chunk = np.tile(
            np.asarray(observation["qpos"], dtype=np.float32), (predicted.shape[0], 1)
        )
