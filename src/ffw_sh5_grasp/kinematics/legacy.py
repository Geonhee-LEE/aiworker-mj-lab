"""이전 단일-site FK 이름을 위한 최소 호환 계층.

실시간 제어는 이 모듈을 사용하지 않고 :class:`KinematicTree`를 직접 공유한다.
삭제된 반복형 ``solve_pose`` API는 제공하지 않는다.
"""
...
class KinematicsSolver:
    """선택한 scalar joint에 대한 단일-site FK/Jacobian 호환 adapter."""
    ...
    def __init__(self, model, site_name, joint_names, *, tree=None):
        ...
        try:
            ...
            raise ValueError(
                "kinematics adapter references an unknown site or joint: "
...
class InverseKinematics(KinematicsSolver):
    """기존 ``InverseKinematics`` 이름만 유지하는 FK adapter."""
