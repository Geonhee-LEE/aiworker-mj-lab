...
MuJoCo는 include와 default가 반영된 topology를 읽는 데만 사용한다. 초기화 뒤의
FK/Jacobian은 ``MjData``나 ``mj_forward`` 없이 qpos 배열만으로 계산한다.
"""
...
class SiteKinematics:
    """Site world pose와 world-frame 6×N geometric Jacobian."""
...
class KinematicJoint:
    """FK에 필요한 scalar joint의 topology와 고정 속성."""
...
class KinematicTree:
    ...
    def _copy_body(cls, model, body_id):
        ...
        joint_ids = tuple(range(joint_address, joint_address + joint_count))
    ...
    def _forward_body(self, qpos, body_id):
        """Root에서 지정 body까지의 pose와 joint world frame을 계산한다."""
        position = np.zeros(3)
        rotation = np.eye(3)
        joint_frames = {}
