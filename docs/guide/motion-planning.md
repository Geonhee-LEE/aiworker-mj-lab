...
- **1차 목표 알고리즘은 `plan_rrt_connect`로 한정한다.** `rrt_star.py`/
  `chomp.py`(PR #4/#5)는 이제 `main`에 병합됐지만, 둘 다 오른팔 7-DOF
  전용으로 짜여 있다(`RightArmSpace` 가정) — `WholeBodySpace`가 실제로
  이 파일들과 그대로 맞물리는지는 인터페이스 계약(위 "정말로 재사용
  가능하다"는 결론의 근거)만 확인했을 뿐 통합 테스트는 아직 없다. RRT*/
  CHOMP whole-body 지원은 Tier 2 착수 시 첫 검증 대상으로 삼는다.

## 다음 단계

P2(shortcut 평활화 + 시간 파라미터화, 데모 연결까지), P3(정식 실행 모듈,
`planning.execution`), P5(RRT* 대안 플래너 + CHOMP류 자세 평활화, 데모
연결까지)는 모두 구현이 끝났다. 벤치마크 하네스(`MP-0013`)도 이미
있으니, 다음은 `MP-0017`(RRT-Connect vs RRT* 50-seed 정식 비교표,
`RESULTS.md`)이다. P7 Tier 2(결합형 `WholeBodySpace`)는 위 타당성
평가에서 "가능하다"고 결론 났지만, 검증할 실제 시나리오(합성 MJCF 포함)가
먼저 준비돼야 착수 의미가 있다 — 사용자의 우선순위 결정을 기다린다.
이후는 자동 연구 루프(`docs/agents.md`)가 `TODO.md`를 보고 진행한다.
