...
!!! info "함께 볼 개발자 가이드"
    현재 저장소의 목표·범위와 모듈 구성은 [프로젝트 개요](../../overview.md),
    검증 명령과 성공 기준은 [테스트](../../testing.md)에 정리돼 있다.
...
1. **live 제어 중 kinematic override 금지** — 로봇을 움직이려고
   `data.qpos[...] = value`로 물리 상태를 직접 덮어쓰지 않는다. 예외는 reset,
   물체 초기 배치(`_reset_can_random()`), 시작 시 사용하지 않는 legacy box를
   장면 밖으로 옮기는 `_disable_legacy_box_asset()`뿐이다.
2. **물리 튜닝 파라미터는 XML에** — 파이썬에서 friction·mass·actuator gain을
   바꿔 동작을 튜닝하지 않는다. `_disable_legacy_box_asset()`은 시작 시 legacy
   box의 `contype`/`conaffinity`와 alpha를 0으로 만들어 can-only workflow에서
   제외하는 명시적 초기화 예외다.
3. **Phase 순서 준수** — 성공 기준을 통과하기 전 다음 Phase 코드를 쓰지 않는다.
4. **Phase 완료마다 git commit + tag**.
5. **Phase마다 headless 테스트 스크립트를 남긴다.**

이 다섯 규칙과 위 초기화 예외의 경계는 현재 코드에서도 유지된다 — Part 13의 버그
...
집중한 단계다. 현재 Phase 6 인터페이스는 Cyclo Control 방식의 MoveL/Bimanual
MoveL 상태와 화면 안 3D gizmo를 제공한다.

## 3.4 현재 상태 스냅샷 {: #part-3-4 }

- 최신 버전 태그: `1.3.0` (`1.2.0`의 자체 FK/Jacobian과 compact UI를 유지하면서
  기구학·WBIK 세부 모듈 분리, 통합 학습 순서와 모방학습·실기 전환 가이드가 포함된다.)
...
[← Part 2](./02-mujoco-model-data.md) · [전체 안내](../index.md#ros2-reading) · [Part 4 →](./04-runtime-architecture.md)
