...
!!! info "현재 실행 경로와 개발자 가이드"
    ...
    [`kinematics.py`](../kinematics.md)의 MJCF 트리가 FK·Jacobian·자세 오차를 공통
    제공한다. [`ik.py`](../ik.md)의 `InverseKinematics` 이름은 Phase 3·4·6 회귀
    테스트와 `tests/record_demo.py`를 위한 호환 진입점이다.
...
flowchart LR
    ...
    STATE["현재 qpos · qvel"] --> TREE["MJCF에서 파싱한 KinematicTree<br>FK · Jacobian 직접 계산"]
    TREE --> LIVE
...
움직이는지는 자체 트리에서 직접 만든 야코비안 \(J\)가 1차 근사로 알려준다:
...
`KinematicsSolver.forward()`가 파싱한 트리에서 pose와 Jacobian을 항상 이 규칙으로
제공하므로 단일 팔 IK와 whole-body IK가 같은 좌표계를 사용한다. 테스트는 pose를
물리 엔진 결과와 대조하고 Jacobian은 각 관절을 \(\pm\epsilon\)만큼 움직인 중앙
유한차분 결과와 대조한다. 따라서 Jacobian 검증에도 다른 Jacobian solver를 쓰지 않는다.
...
flowchart TD
    ...
    TRY --> FK["tree FK로 실제 비용 C(q_try) 계산"]
...
| `KinematicsSolver.solve_pose()` (`InverseKinematics` 호환 이름) | Phase 3·4·6 테스트, `tests/record_demo.py` | 호출자가 지정한 반복 수 안에서 tree FK, DLS와 line search를 반복한다. |
