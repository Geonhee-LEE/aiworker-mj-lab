...
| `robot_state_publisher` + tf2 | `KinematicTree`의 직접 FK | 컴파일된 MJCF body–joint–site 트리를 순회하며 world pose를 계산한다. tf 발행은 없다 |
...
| `rqt`/dynamic reconfigure 도구 창 | `teleop_ui.py`의 기능별 ImGui 플랫폼 창 | 3D 주 창 밖의 별도 OS 창으로 이동·크기 조절·숨김 가능 |
