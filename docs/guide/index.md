...
| 모방학습 시작·문서 선택 | [모방학습 전체 안내](il/index.md) | 목적별 실행·이론 경로 |
| 모방학습 명령 실행 | [명령어 레퍼런스](../imitation-commands.md) | IL camera/env + training |
| IL 코드 수정·확장 | [모방학습 코드 구조](imitation-code-structure.md) | `test_il_*.py` 전체 |
...
모방학습은 실행·결과·이론 문서가 별도의 최상위 메뉴로 분리되어 있다. 공개 policy를
실행할지, 직접 학습할지, ACT를 공부할지에 따라 [모방학습 전체 안내](il/index.md)에서
경로를 선택한다.
