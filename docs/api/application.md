...
위 세 양손 메서드는 UI 명령과 solver 기준 갱신을 한 동작으로 묶는 앱용 진입점이다.
순수 좌표 변환은 `application.targets` 함수를 직접 호출한다.
...
| `sync_marker_visibility(app)` | 캡처 상태를 판정해 실제 geom/site alpha에 반영한다 | 앱 | 모델 표시 색상 변경 |
