#!/usr/bin/env bash
# daily_brief — cron wrapper. 실제 로직은 scripts/prompts/brief.md 에 있다.
set -euo pipefail
...
LOG="${LOG_DIR}/daily_brief-$(TZ=Asia/Seoul date +%Y-%m-%d).log"
...
{
  echo "=== daily_brief start $(date -Iseconds) ==="
  claude -p "$(cat "${PROMPT}")" \
    --output-format text \
    --permission-mode acceptEdits \
    --allowedTools "${ALLOWED[@]}"
  rc=$?
  echo "=== daily_brief end $(date -Iseconds) rc=${rc} ==="
  exit ${rc}
} >> "${LOG}" 2>&1
