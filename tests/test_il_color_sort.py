...
def _place_can_at_site(env, site_name):
    ...
    env.data.qpos[env.task.can_qpos:env.task.can_qpos + 3] = (
...
def test_legacy_scenario_keeps_red_bin_disabled():
    with AIWorkerMujocoEnv(
            render_images=False, seed=3, task_name="can_to_box") as env:
...
def test_color_sort_randomizes_visual_only_and_uses_matching_target():
    with AIWorkerMujocoEnv(
            ...
            task_name="can_color_sort") as env:
        ...
        assert np.allclose(
            env.data.site_xpos[spawn_site], [0.4055, -0.05, 0.7866])
        for seed in range(32):
            observation = env.reset(seed=seed)
            variant = observation["task"]["object_variant"]
            variants.setdefault(variant, seed)
            assert observation["task"]["target_label"] == (
                EXPECTED_TARGETS[variant])
            assert np.linalg.norm(
                env.initial_can_position[:2]
                - env.data.site_xpos[spawn_site, :2]
        ...
        assert np.array_equal(
            env.model.geom_friction[can_geom], physical_signature[1])
        ...
        table_geom = mujoco.mj_name2id(
            env.model, mujoco.mjtObj.mjOBJ_GEOM, "table")
        ...
        for body_name, expected_position in expected_positions.items():
            ...
            wall_geoms = [
                geom_id for geom_id in bin_geoms
                if not (mujoco.mj_id2name(
                    env.model, mujoco.mjtObj.mjOBJ_GEOM,
                    int(geom_id)) or "").endswith("floor")
            ]
            assert np.allclose(
                env.model.geom_size[wall_geoms, 2], 0.08 * 2.0 / 3.0)
            outer_edge = abs(
                env.model.body_pos[body_id, 1]
                - env.model.geom_pos[table_geom, 1]) + max(
                abs(env.model.geom_pos[geom_id, 1])
                + env.model.geom_size[geom_id, 1]
                for geom_id in bin_geoms)
            assert outer_edge < env.model.geom_size[table_geom, 1]
        blue_body = mujoco.mj_name2id(
            env.model, mujoco.mjtObj.mjOBJ_BODY, "target_bin")
        red_body = mujoco.mj_name2id(
            env.model, mujoco.mjtObj.mjOBJ_BODY, "target_bin_red")
        assert np.isclose(
            env.model.body_pos[red_body, 1]
            - env.model.body_pos[blue_body, 1], 0.44)
        assert np.isclose(
            0.5 * (env.model.body_pos[blue_body, 1]
                   + env.model.body_pos[red_body, 1]), -0.05)
        ...
        for variant, seed in variants.items():
            ...
            assert np.allclose(
                metadata["bin_body_positions"]["target_bin"],
                [0.5355, -0.27, 0.7391])
            assert np.allclose(
                metadata["bin_body_positions"]["target_bin_red"],
                [0.5355, 0.17, 0.7391])
...
def test_color_sort_recorder_can_target_underrepresented_variants():
    with AIWorkerMujocoEnv(
            ...
            randomize_bin_colors=True) as env:
        variants = set()
        layouts = set()
        target_bodies = set()
        for seed in range(64):
            ...
            target_bodies.add(mujoco.mj_id2name(
    ...
    with AIWorkerMujocoEnv(
        ...
        for site_name, bin_body_name, direction in cases:
            ...
            site_id = mujoco.mj_name2id(
                env.model, mujoco.mjtObj.mjOBJ_SITE, site_name)
            wall_geoms = {
                int(geom_id) for geom_id in _geom_ids(
                    env.model, bin_body_name)
                if not (mujoco.mj_id2name(
                    env.model, mujoco.mjtObj.mjOBJ_GEOM,
                    int(geom_id)) or "").endswith("floor")
            }
            ...
            for _ in range(250):
                ...
                touched_wall = touched_wall or any(
                    ...
                    and bool(wall_geoms.intersection(
...
def test_color_sort_starts_right_arm_ten_centimeters_higher_without_bin_contact():
    with AIWorkerMujocoEnv(
            render_images=False, seed=0,
            task_name="can_to_box") as legacy_env, AIWorkerMujocoEnv(
                render_images=False, seed=0,
                task_name="can_color_sort") as color_sort_env:
        legacy_pose = legacy_env.get_observation()["ee_pose"]["right"]
        color_sort_pose = color_sort_env.get_observation()["ee_pose"]["right"]

        assert np.allclose(
            color_sort_pose[:3] - legacy_pose[:3], [0.0, 0.0, 0.1],
            atol=1e-7)
        assert np.isclose(
            abs(np.dot(color_sort_pose[3:], legacy_pose[3:])), 1.0,
            atol=1e-9)
