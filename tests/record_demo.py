...
class FrameGrabber:
    ...
    def save(self, path):
        ...
        # 몇 배 줄일 수 있다.
        quantized = [
            f.quantize(colors=128, method=Image.MEDIANCUT) for f in self.frames
        ]
        quantized[0].save(
            path,
            save_all=True,
...
def main():
    ...
    out_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else str(REPO_ROOT / "docs" / "assets" / "demo.gif")
    )
    ...
    print(
        f"net_lift={net_lift * 100:.2f}cm grasped={grasp.is_grasped(model, data, side='r')}"
    )
