...
def _set_hand_base_target(app, side, base_pos):
    ...
    app.targets[f"pos_{side}"] = (
        np.array(base_pos) - app.home_pos_local[side]
    ).tolist()
...
def run_cyclo_marker_jog_gate():
    ...
    app.arm_mode["l"] = "fk"
    teleop_ui._apply_cartesian_jog(
        app, "both", pos_delta=(1.0, 1.0, 1.0), rpy_delta=(100.0, 100.0, 100.0)
    )
    ...
    move_marker_ok = (
        teleop_ui._active_marker_choices(app)
        == (("r", "Right goal"), ("l", "Left goal"))
    ...
    print(
        f"Cyclo marker jog gate: both_updates={both_ok} "
...
def run_initial_ik_target_origin_gate():
    ...
    offsets_zero = np.allclose(app.targets["pos_r"], [0.0, 0.0, 0.0]) and np.allclose(
        app.targets["pos_l"], [0.0, 0.0, 0.0]
    )
    pose_matches = True
    reports = []
    for side in ("r", "l"):
        ...
        reports.append(
            f"{side}: pos_err={pos_err * 1000:.6f}mm quat_dot={quat_dot:.12f}"
        )
    ok = offsets_zero and pose_matches
    print(
        f"Initial IK target origin gate: offsets_zero={offsets_zero} "
...
def run_cyclo_bimanual_virtual_object_gate():
    ...
    vo_pos = teleop_targets.local_to_world_pos(app, app.targets["virtual_object_pos"])
    ...
    print(
        f"Cyclo bimanual virtual object gate: capture={capture_ok} "
...
def run_cyclo_3d_gizmo_pose_gate():
    ...
    teleop_targets.set_gizmo_target_world_pose(app, "r", world_pos, world_quat)
    ...
    teleop_targets.set_gizmo_target_world_pose(app, "virtual", vo_pos, vo_quat)
    ...
    print(
        f"Cyclo 3D gizmo pose gate: roundtrip={roundtrip_ok} "
...
def run_bimanual_marker_visibility_gate():
    ...
    print(
        f"Bimanual marker visibility gate: initial_hidden={hidden_initial} "
...
def run_numeric_target_marker_sync_gate():
    ...
    print(
        f"Numeric target -> marker sync gate: {'; '.join(reports)}: "
        f"{'OK' if ok else 'FAIL'}"
    )
    return ok
...
def run_whole_body_toggle_gate():
    ...
    app.toggle_whole_body_control()
    hand_on = {side: teleop_targets.target_world_pose(app, side) for side in ("r", "l")}
    ...
    captured_round_trip = all(
        ...
        and abs(
            np.dot(
                captured_before[side][1],
                teleop_targets.target_world_pose(captured_app, side)[1],
            )
        )
        > 1.0 - 1e-10
        for side in ("r", "l")
    ...
    captured_round_trip &= (
        ...
        and abs(np.dot(captured_virtual_before[1], captured_virtual_after[1]))
        > 1.0 - 1e-10
    )
    ...
    ok = (
        off_mode
        and off_pose_preserved
        and stale_command_cleared
        and smoothed_synced
        and round_trip
        and captured_round_trip
        and off_integration
    )
    print(
        f"Whole-body toggle gate: off={off_mode} off_pose={off_pose_preserved} "
...
def run_collision_visualization_gate():
    ...
    app = _make_sim_only_app()
    initially_off = (
        not app.collision_viz and teleop_render.collision_visualization_data(app) == ()
    )
    ...
    data_ok = (
        ...
        len(constraints) >= 1
        and all(
            constraint.distance <= app.whole_body_solver.collision_buffer + 1e-12
            for constraint in constraints
        )
        and all(
            np.isfinite(constraint.point_a).all()
    ...
    overlay_ok = (
        ...
        and sum(
            int(app.scene.geoms[i].type) == int(mujoco.mjtGeom.mjGEOM_LINE)
            for i in range(app.scene.ngeom)
        )
        == len(constraints)
        and sum(
            int(app.scene.geoms[i].type) == int(mujoco.mjtGeom.mjGEOM_SPHERE)
            for i in range(app.scene.ngeom)
        )
        == 2 * len(constraints)
    )
    ...
    colors_distinct = not np.array_equal(
        teleop_render._collision_color(-0.001, safe_distance),
    ...
    app.toggle_collision_visualization()
    toggles_off = (
        not app.collision_viz and not teleop_render.collision_visualization_data(app)
    )
    ok = initially_off and data_ok and overlay_ok and colors_distinct and toggles_off
    print(
        f"Collision visualization gate: initial_off={initially_off} "
...
def run_task_space_input_gate():
    ...
    applied, message = task_space.apply_target(app, side, desired_position, desired_rpy)
    ...
    conversion_ok = (
        ...
        and abs(float(np.dot(converted_quaternion, desired_quaternion))) > 1.0 - 1e-10
    )
    ...
    side_ok, _ = task_space.apply_target(app, "invalid", initial.position, desired_rpy)
    rejects_invalid = (
        not finite_ok
        and not side_ok
...
def run_application_state_gate():
    ...
    app.steps_per_frame = max(1, round(app.frame_dt / app.model.opt.timestep))
    ...
    snapshot_ok = (
        ...
        and all(
            not pose.flags.writeable for pose in observation.hand_positions.values()
        )
    )
    app.data.qpos[base_x] -= 0.01
    mujoco.mj_forward(app.model, app.data)
    ...
    print(
        f"Application state gate: bindings={bindings_ok} "
...
def run_split_ui_and_tree_gate():
    ...
    tree_ok = (
        ...
        and all(
            tree.bodies[child].parent_id == parent
            for parent, children in enumerate(tree.children_by_body)
            for child in children
        )
        and all(
            joint.kind_name in {"free", "ball", "slide", "hinge"}
            for joint in tree.joints
        )
    )
    ok = windows_ok and tree_ok
    print(
        f"Tabbed UI/tree gate: workspaces={len(windows)} compact={windows_ok} "
