...
from ffw_sh5_grasp.imitation.act.dataset_loader import episode_paths
...
def test_one_epoch_training_outputs():
    with tempfile.TemporaryDirectory() as directory:
        ...
        write_episode(
            dataset_dir / "episode_000000.hdf5",
            EpisodeData(
                qpos=np.zeros((3, 16), np.float32),
                qvel=np.zeros((3, 16), np.float32),
                images=images,
                action=np.zeros((3, 16), np.float32),
                debug={},
                attrs={"seed": 1},
            ),
        )
...
def test_episode_paths_can_freeze_a_dataset_prefix():
    with tempfile.TemporaryDirectory() as directory:
        root = pathlib.Path(directory)
        paths = [root / f"episode_{index:06d}.hdf5" for index in range(4)]
        for path in paths:
            path.touch()
        assert episode_paths(root, episode_count=2) == paths[:2]
        with np.testing.assert_raises_regex(ValueError, "only 4 are available"):
            episode_paths(root, episode_count=5)
