...
def run_ros_free_dependency_gate():
    ...
    ok = not violations
    print(
        f"ROS-free dependency gate: forbidden imports={violations}: "
...
def run_tree_kinematics_dependency_gate():
    ...
    solver_classes = {
        filename: {
            node.name
            for node in modules[filename].body
            if isinstance(node, ast.ClassDef)
        }
        for filename in expected_classes
    }
    solver_forbidden = {"MjData", "mj_forward"}
    runtime_forbidden = {"mj_forward", "mj_jacSite", "mj_jac", "site_xpos", "site_xmat"}
    violations = set()
    ...
    expected_functions = {
        ...
        "ffw_sh5_grasp/control/bimanual.py": {"capture_reference", "rigid_grasp_task"},
    }
    functions_ok = all(
        function_names
        <= {
    ...
    print(
        f"Tree-kinematics dependency gate: forbidden runtime FK={violations}: "
        f"{'OK' if ok else 'FAIL'}"
    )
    return ok
...
def run_shared_pose_task_gate():
    ...
    position_ok = np.allclose(error.position, target_position - current_position)
    ...
    print(
        f"Shared pose-task gate: position={position_ok} "
...
def run_swerve_kinematics_gate():
    ...
    for _ in range(100):
        ...
        current = {
            name: rng.uniform(*base_teleop.STEER_RANGE) for name in base_teleop.WHEELS
        }
        ...
        feasible &= all(
            base_teleop.STEER_RANGE[0] <= angle <= base_teleop.STEER_RANGE[1]
    ...
    print(
        f"Swerve kinematics gate: feasible={feasible} roundtrip={max_roundtrip:.2e} "
...
def run_box_qp_gate():
    ...
    for _ in range(25):
        ...
        hessian, linear = bounded_optimization.least_squares_to_qp(matrix, vector)
        solution = bounded_optimization.bounded_quadratic_program(
            hessian, linear, lower, upper
        )
        qp_objective = float(0.5 * solution @ hessian @ solution + linear @ solution)
        ...
        least_squares_objective = float(np.linalg.norm(matrix @ solution - vector) ** 2)
        worst_conversion_error = max(
            worst_conversion_error, abs(objective - least_squares_objective)
        )
    ok = worst_gap < 1e-9 and worst_conversion_error < 1e-10
    print(
        f"Box-QP gate: objective_gap={worst_gap:.2e} "
...
def run_explicit_qp_path_gate():
    ...
    whole_body_path = REPO_ROOT / "src" / "ffw_sh5_grasp" / "control" / "whole_body.py"
    optimization_path = (
    ...
    optimization_tree = ast.parse(
        optimization_path.read_text(encoding="utf-8"), filename=str(optimization_path)
    )
    ...
    solver_functions = {
        node.name
        for node in optimization_tree.body
        if isinstance(node, ast.FunctionDef)
    }
    required_functions = {
        "least_squares_to_qp",
    ...
    ok = (
        not imported_optimization
        and not missing
        and {"solve", "enforce_constraints"} <= solver_calls
        and not forced_base_override
    )
    print(
        f"Solver ownership gate: optimization_import={imported_optimization} "
...
def run_selectable_ik_methods_gate():
    ...
    barriers = all(
        (barrier_matrix @ solution).item() >= -0.201 for solution in solutions.values()
    )
    distinct = not np.allclose(solutions["pseudoinverse"], solutions["dls"], atol=1e-8)
    ...
    fixed_solutions = {
        method: kinematics.DifferentialIKSolver(method, dls_damping=0.01).solve(
    ...
    for method in ("pseudoinverse", "dls"):
        ...
        for _ in range(20):
            ...
            active_solution = kinematics.DifferentialIKSolver(
                method, dls_damping=damping
            ).solve(active_matrix, active_vector, active_lower, active_upper)
            if method == "dls":
                reference_matrix = np.vstack([active_matrix, damping * np.eye(6)])
                reference_vector = np.concatenate([active_vector, np.zeros(6)])
            else:
            ...
            reference = bounded_optimization.bounded_quadratic_program(
                reference_hessian, reference_linear, active_lower, active_upper
            )

            def objective(value):
                return np.linalg.norm(reference_matrix @ value - reference_vector) ** 2
    ...
    ok = bounded and barriers and distinct and fixed_reallocated and active_set_optimal
    print(
        f"Selectable IK methods gate: bounded={bounded} barriers={barriers} "
...
def run_arm_only_selectable_methods_gate(model):
    ...
    solver = whole_body_ik.WholeBodyIK(
        model,
        {side: f"grasp_target_{side}" for side in ("r", "l")},
        ARMS,
    ...
    for method in ("pseudoinverse", "dls", "qp"):
        ...
        command = solver.solve(data, targets, 0.04, whole_body_enabled=False)
    ...
    print(
        f"Arm-only selectable methods gate: rates={rates} body_fixed={body_fixed} "
...
def run_qp_velocity_normalization_gate(model):
    ...
    normalized = np.allclose(unit_cost, 1.0 / np.square(probe.velocity_limits))
    ...
    rigid_cost = pose_tasks.normalized_weights(
        [probe.rigid_grasp_position_weight, probe.rigid_grasp_orientation_weight],
        [probe.max_task_linear_speed, probe.max_task_angular_speed],
    )
    ...
    for strength in (1e-4, 1e3):
        ...
        command_norms[strength] = float(np.linalg.norm(command.generalized_velocity))
    sensitive = command_norms[1e3] < 0.25 * command_norms[1e-4]
    ok = normalized and defaults_preserved and sensitive
    print(
        f"QP velocity normalization gate: normalized={normalized} "
...
def run_joint_limit_cbf_gate(model):
    ...
    print(
        f"Joint-limit CBF gate: approach_bound={upper[index]:.4f} "
...
def _self_collision_fixture(model):
    ...
    pair = next(
        pair
        for pair in solver.collision_pairs
...
def run_collision_gradient_gate(model):
    ...
    for index, qpos_adr in enumerate(solver.qpos_adrs):
        ...
        for sign in (-1.0, 1.0):
            ...
            perturbed = kinematics.collision_distance_gradient(
                model, scratch, pair, solver.kinematic_tree, solver.joint_ids, 0.10
            )
    ...
    mujoco.mj_forward(model, data)
    hand_pair = next(
        pair for pair in solver.collision_pairs if "hx5_r_base/hx5_l_base" in pair.name
    )
    ...
    for index, qpos_adr in enumerate(solver.qpos_adrs):
        ...
        for sign in (-1.0, 1.0):
            ...
            mujoco.mj_forward(model, scratch)
            perturbed = kinematics.collision_distance_gradient(
                model, scratch, hand_pair, solver.kinematic_tree, solver.joint_ids, 0.10
            )
            distances.append(perturbed.distance)
        hand_numerical[index] = (distances[1] - distances[0]) / (2.0 * step)
    hand_error = float(np.max(np.abs(hand_result.gradient - hand_numerical)))
    ...
    ok = (
        result.distance < solver.collision_buffer
        and error < 1e-6
    ...
    print(
        f"Collision gradient gate: pairs={len(solver.collision_pairs)} "
...
def run_self_collision_cbf_gate(model):
    ...
    pair = next(
        pair
        for pair in probe_solver.collision_pairs
        if "hx5_r_base/hx5_l_base" in pair.name
    )
    result = kinematics.collision_distance_gradient(
        model, data, pair, probe_solver.kinematic_tree, probe_solver.joint_ids, 0.10
    )
    ...
    constraint = next(item for item in constraints if item.name == pair.name)
    ...
    print(
        f"Self-collision CBF gate: d={constraint.distance * 1000:.1f}mm "
...
def run_table_collision_cbf_gate(model):
    ...
    safe_violation = float(
        np.max(np.maximum(lower - matrix @ safe.generalized_velocity, 0.0))
    )
    ...
    ok = (
        len(constraints) >= 2
        and workspace_only
        and free_violation > 0.10
        and safe_violation < 1e-3
    ...
    print(
        f"Table collision CBF gate: active={len(constraints)} "
...
def run_collision_inactive_regression_gate(model):
    ...
    safe = safe_solver.solve(data, targets, 0.04)
    difference = float(
        np.max(np.abs(free.generalized_velocity - safe.generalized_velocity))
    )
    ok = (
        not safe.active_collision_pairs
        and difference < 1e-12
        and np.isinf(safe.minimum_collision_distance)
    )
    print(
        f"Inactive collision regression gate: active={len(safe.active_collision_pairs)} "
...
def run_rigid_grasp_gate(model):
    ...
    states = {side: rigid_solver.site_state(data, side) for side in ("r", "l")}
    ...
    free_residual = float(
        np.linalg.norm(grasp_jacobian @ free.generalized_velocity - desired_velocity)
    )
    rigid_residual = float(
        np.linalg.norm(grasp_jacobian @ rigid.generalized_velocity - desired_velocity)
    )
    ok = free_residual > 0.05 and rigid_residual < 0.25 * free_residual
    print(
        f"Rigid-grasp gate: relative_twist={free_residual:.3f}->{rigid_residual:.3f}: "
        f"{'OK' if ok else 'FAIL'}"
    )
    return ok
...
def run_rigid_grasp_physical_gate():
    ...
    app.fk_q_deg = {
        side: np.degrees(q_des).tolist() for side, q_des in app.q_des.items()
    }
    app.frame_dt = 0.04
    ...
    start_states = {
        side: app.whole_body_solver.site_state(app.data, side) for side in ("r", "l")
    }
    start_midpoint = 0.5 * (start_states["r"].position + start_states["l"].position)
    ...
    position_drift = float(np.linalg.norm(position_right - reference["position_right"]))
    ...
    print(
        f"Rigid-grasp physical gate: drift={position_drift * 1000:.1f}mm/"
...
def run_world_anchor_gate():
    ...
    hand_fixed = (
        np.linalg.norm(hand_after[0] - hand_before[0]) < 1e-12
        and abs(abs(np.dot(hand_after[1], hand_before[1])) - 1.0) < 1e-12
    )
    virtual_fixed = (
        np.linalg.norm(virtual_after[0] - virtual_before[0]) < 1e-12
    ...
    print(
        f"World target anchor gate: hand_fixed={hand_fixed} virtual_fixed={virtual_fixed} "
...
def run_manual_handover_gate():
    ...
    carried_positions = all(
        np.linalg.norm(
            targets_after[side][0] - expected_position(targets_before[side][0])
        )
        < 1e-12
        for side in ("r", "l")
    )
    carried_virtual = (
        np.linalg.norm(virtual_after[0] - expected_position(virtual_before[0])) < 1e-12
    )
    ...
    for side in ("r", "l"):
        ...
        carried_orientations &= (
            abs(abs(np.dot(expected_quat, targets_after[side][1])) - 1.0) < 1e-12
        )
    ...
    ok = (
        carried_positions
        and carried_virtual
        and carried_orientations
        and no_return
        and wbik_resumes
    )
    print(
        f"Manual handover gate: carried_pos={carried_positions} "
...
def run_manual_release_physical_gate():
    ...
    ok = (
        return_distance < 0.005
        and stop_time is not None
        and stop_time < 0.5
        and wheel_stop_time is not None
        and wheel_stop_time < 0.5
    )
    print(
        f"Manual release physical gate: brake_excursion={braking_excursion * 1000:.1f}mm "
...
def run_whole_body_solver_gate(model):
    ...
    both_arms_used = all(
        np.linalg.norm(qdot[solver.side_indices[s]]) > 1e-3 for s in ("r", "l")
    )
    ...
    mujoco.mj_forward(model, scratch)
    after_error = sum(
        np.linalg.norm(targets[s][0] - scratch.site_xpos[sites[s]]) for s in sites
    )
    ...
    ok = (
        ...
        and lift_used
        and both_arms_used
        and within_velocity_limits
        and descent
    )
    print(
        f"Whole-body solver gate: read_only={read_only} base={base_used} lift={lift_used} "
...
def run_base_participation_gate(model):
    ...
    reduced = np.linalg.norm(limited.generalized_velocity[:3]) < np.linalg.norm(
        full.generalized_velocity[:3]
    )
    base_exactly_off = np.array_equal(disabled.generalized_velocity[:3], np.zeros(3))
    lift_and_arms_remain = np.linalg.norm(disabled.generalized_velocity[3:]) > 1e-3
    ok = (
        limited_respects_bound and reduced and base_exactly_off and lift_and_arms_remain
    )
    print(
        f"Base participation gate: limited_bound={limited_respects_bound} "
...
def run_arm_only_solver_gate(model):
    ...
    twist_zero = (
        command.base_twist.vx == 0.0
        and command.base_twist.vy == 0.0
    ...
    print(
        f"Arm-only solver gate: body_qdot_zero={body_pinned} twist_zero={twist_zero} "
...
def run_solver_latency_gate(model, solve_count=200):
    ...
    ok = milliseconds < 5.0
    print(
        f"Whole-body latency gate: {milliseconds:.3f}ms/solve (<5ms): "
        f"{'OK' if ok else 'FAIL'}"
    )
    return ok
...
def run_randomized_whole_body_gate(model, trial_count=40):
    ...
    print(
        f"Randomized WBIK gate: {successes}/{trial_count} descent+read-only+bounded "
        f"worst_ratio={worst_ratio:.3f}: {'OK' if ok else 'FAIL'}"
    )
    return ok


def _physical_whole_body_trial(
    model, delta, duration=1.5, yaw_delta=0.0, rotate_positions=False
):
    ...
    controllers = {
        side: arm_control.ArmTorqueController(model, ARMS[side]) for side in ("r", "l")
    }
    drive = base_teleop.SwerveDrive()

    steer_qadrs = {
        wheel: model.jnt_qposadr[
            mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, f"{wheel}_steer_joint")
        ]
        for wheel in base_teleop.WHEELS
    }
    drive_dofs = {
        wheel: model.jnt_dofadr[
            mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, f"{wheel}_drive_joint")
        ]
        for wheel in base_teleop.WHEELS
    }
    steer_aids = {
        wheel: mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, f"{wheel}_steer")
        for wheel in base_teleop.WHEELS
    }
    drive_aids = {
        wheel: mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, f"{wheel}_drive")
        for wheel in base_teleop.WHEELS
    }
    ...
    position_ramp_frames = int(
        np.ceil(
            max(np.max(np.abs(targets[s][0] - start_targets[s][0])) for s in sites)
            / 0.03
        )
    )
    ...
    for frame in range(round(duration / frame_dt)):
        ...
        steering = {
            wheel: float(data.qpos[qadr]) for wheel, qadr in steer_qadrs.items()
        }
        wheel_velocity = {
            wheel: float(data.qvel[dof]) for wheel, dof in drive_dofs.items()
        }
...
def run_physical_whole_body_gate(model):
    ...
    for name, result in trials.items():
        ...
        print(
            f"Physical WBIK {name}: error={result['initial_error'] * 1000:.1f}->"
