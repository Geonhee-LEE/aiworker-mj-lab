...
from ffw_sh5_grasp.imitation.act.modular_representations import (  # noqa: E402
    create_representation,
)
from ffw_sh5_grasp.imitation.act.modular_trainer import (  # noqa: E402
    train_modular,
)
from ffw_sh5_grasp.imitation.act.modular_training_config import (  # noqa: E402
    ModularACTTrainingConfig,
)
...
def _episode(path, length=3):
    with AIWorkerMujocoEnv(
        ...
        full_qpos = np.repeat(
            observation["debug"]["full_qpos"][None], length, axis=0)
        images = {
            name: np.zeros((length, 32, 32, 3), dtype=np.uint8)
            for name in ("cam_high", "cam_right_wrist")
        }
        return write_episode(path, EpisodeData(
            qpos=qpos, qvel=qvel, ee_pose=ee_pose,
...
def test_modular_task_smoke_training_writes_representation_metadata():
    with tempfile.TemporaryDirectory() as directory:
        ...
        run_dir = train_modular(config_path)
...
if __name__ == "__main__":
    ...
    test_modular_task_smoke_training_writes_representation_metadata()
